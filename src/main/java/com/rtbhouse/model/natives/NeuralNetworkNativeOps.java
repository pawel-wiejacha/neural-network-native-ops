...
/**
 * <p>Utility class with few operations useful when dealing with neural networks: ReLU, linearForward and simple
...
public final class NeuralNetworkNativeOps {
    ...
    public static void ELU(FloatBuffer inOut, float alpha) {
        ELU(inOut, inOut.capacity(), alpha);
    }
     ...
     * </pre>
     *
     * @param A     input 2d matrix with logical dimensions: {@code ySize} x {@code xSize} (ro)
     * @param x     input vector (ro)
     * @param y     input/output vector (rw)
     * @param xSize size of input vector
     * @param ySize size of input/output vector
     * @see <a
     * href=
     ...
     * </pre>
     *
     * @param A - input        k x m     matrix
     * @param B - input            m x n matrix
     * @param Y - input/output k   x   n matrix
     * @param k
     * @param m
     * @param n
     * @see <a href="http://www.netlib.org/lapack/explore-html/db/dc9/group__single__blas__level3.html#gafe51bacb54592ff5de056acabd83c260"
     ...
     * </pre>
     *
     * @param transpose  whether {@code weights} should be transposed before multiplication
     * @param weights    weights 2d matrix with logical dimensions: {@code outputSize} x {@code inputSize} (after optional transposition) (ro)
     * @param biases     bias vector with size {@code outputSize} (ro)
     * @param input      input vector with size {@code inputSize} (ro)
     * @param output     output vector with size {@code outputSize} (write only)
    ...
    public static void linearForward(int transpose, FloatBuffer weights, FloatBuffer biases, FloatBuffer input, FloatBuffer output) {
        linearForward(transpose, weights, biases, input, output, input.capacity(), output.capacity());
    }

    /**
     * <p>Forward operation for a single linear neural-network layer:
     * Output contents is discarded and overwritten. Other buffers are read-only.
     * </p>
     * <pre>
     ...
     *
     * @param transpose     whether {@code weights} should be transposed before multiplication
     * @param weights       weights 2d matrix with logical dimensions: {@code inputRowSize} x {@code outputRowSize} (after optional transposition) (ro)
     * @param biases        bias vector with size {@code outputRowSize} (ro)
     * @param input         input matrix with size {@code inputRowSize} x {@code batchSize} (ro)
     * @param output        output where matrix with size {@code outputSize} x {@code batchSize} will be written (write only)
     * @param inputRowSize  number of floats in each row of the input
     * @param outputRowSize number of floats in each row of the output
     * @param batchSize     number of rows in input, thus in output as well
     */
